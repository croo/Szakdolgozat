...
public class RdfDatabaseGenerator
{
	...
	private void unZipper() throws IOException
	{
		...
		try {
			...
			System.err.println("Security Exception. Cannot read or write :" + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	private void downloadSource(String databaseFile) throws IOException
	{
		String url = SystemProperties.GetInstance().getFileLocation("mav.source.zip.url");
		saveUrl("MavSourceData.zip", url);

	}

	public void generateRdfDatabase(String databaseFile)
	{
		downloadSource(databaseFile);
